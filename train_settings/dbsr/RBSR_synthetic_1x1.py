...
from models.RBSR_train import RBSR_1x1
import actors.dbsr_actors as dbsr_actors
from trainers import SimpleTrainer_1x1
...
def run(settings):
    ...
    net = RBSR_1x1(align_type=settings.align_type)
    ...
    objective = {'rgb': PixelWiseError(metric='l1', boundary_ignore=None), 
                 'feat': PixelWiseError(metric='charbonnier', boundary_ignore=None),
                 'psnr': PSNR(boundary_ignore=40)}
    loss_weight = {'rgb': 1.0, 'feat': 0.1}
    actor = dbsr_actors.DBSRSyntheticActor_1x1(net=net, objective=objective, loss_weight=loss_weight)
    ...
    trainer = SimpleTrainer_1x1(actor, [loader_train, loader_val], optimizer, settings, lr_scheduler)
